...
impl<'a> Business<'a> {
    ...
    fn decode(&self, dto: auth_service::Role) -> Model {
    ...
    fn encode(&self, model: Model) -> auth_service::Role {
        auth_service::Role {
            id: model.id,
            name: model.name,
            nature: model.nature,
    ...
    pub async fn list(
        &self,
        filter: Option<auth_service::RoleFilter>,
        mut sorts: Vec<public::Sort>,
        pager: Option<public::Pager>,
    ) -> auth_service::ListRoleReply {
        ...
        if let Some(filter) = filter {
            ...
            if filter.natures.is_empty().not() {
        ...
        let header = if pager.disabled {
            ...
            match query.all(self.db).await {
                ...
                Err(e) => common::header::err_reply(
                    ...
                    ECode::AuthServiceErrorListRole,
                    format!("list role failed: {}", e),
                ),
            }
        } else {
            // 分页
        ...
        auth_service::ListRoleReply { header, pager: Some(pager), data: dtos }
    }

    pub async fn operate(&self, operate: Operation, data: Option<auth_service::Role>) -> auth_service::OperateRoleReply {
        let (header, data) = match data {
            None => {
                (common::header::err_reply(
                    self.header.trace_id,
                    ECode::AuthServiceErrorOperateRole,
        ...
        auth_service::OperateRoleReply { header, data }
    }

    pub async fn batch_operate(&self, operate: Operation, data: Vec<auth_service::Role>) -> auth_service::BatchOperateRoleReply {
        ...
        auth_service::BatchOperateRoleReply { header }
    }


    // 插入
    async fn insert(&self, dto: auth_service::Role) -> Result<Option<auth_service::Role>, DbErr> {
    ...
    // 更新
    async fn update(&self, dto: auth_service::Role) -> Result<Option<auth_service::Role>, DbErr> {
    ...
    // 物理删除
    async fn delete_real(&self, id: i32) -> Result<Option<auth_service::Role>, DbErr> {
        Entity::delete_by_id(id).exec(self.db).await?;
        Ok(None)
    }

    // 保存并更新
    async fn save(&self, mut data: auth_service::Role) -> Result<Option<auth_service::Role>, DbErr> {
    ...
    // 批量删除
    async fn batch_delete(&self, dtos: Vec<auth_service::Role>) -> Result<Option<i32>, DbErr> {
        let mut ids = vec![];
        for dto in dtos {
            ids.push(dto.id);
        }
