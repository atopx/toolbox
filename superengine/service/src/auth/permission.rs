...
impl<'a> Business<'a> {
    ...
    fn decode(&self, dto: auth_service::Permission) -> Model {
    ...
    fn encode(&self, model: Model) -> auth_service::Permission {
    ...
    pub async fn list(
        &self,
        filter: Option<auth_service::PermissionFilter>,
        mut sorts: Vec<public::Sort>,
        pager: Option<public::Pager>,
    ) -> auth_service::ListPermissionReply {
        ...
        if let Some(filter) = filter {
            ...
            if filter.access_ids.is_empty().not() {
        ...
        let header = if pager.disabled {
            ...
            match query.all(self.db).await {
                ...
                Err(e) => common::header::err_reply(
                    ...
                    ECode::AuthServiceErrorListPermission,
                    format!("list permission failed: {}", e),
                ),
            }
        } else {
            // 分页
        ...
        auth_service::ListPermissionReply { header, pager: Some(pager), data: dtos }
    }

    pub async fn operate(&self, operate: Operation, data: Option<auth_service::Permission>) -> auth_service::OperatePermissionReply {
        ...
        auth_service::OperatePermissionReply { header, data }
    }

    pub async fn batch_operate(&self, operate: Operation, data: Vec<auth_service::Permission>) -> auth_service::BatchOperatePermissionReply {
        ...
        auth_service::BatchOperatePermissionReply { header }
    }


    // 插入
    async fn insert(&self, dto: auth_service::Permission) -> Result<Option<auth_service::Permission>, DbErr> {
    ...
    // 更新
    async fn update(&self, dto: auth_service::Permission) -> Result<Option<auth_service::Permission>, DbErr> {
        let value: Option<Model> = Entity::find_by_id(dto.id).one(self.db).await?;
        let mut active: ActiveModel = value.unwrap().into();
        active.access_id = Set(dto.access_id);
    ...
    // 逻辑删除
    async fn delete(&self, id: i32) -> Result<Option<auth_service::Permission>, DbErr> {
    ...
    // 物理删除
    async fn delete_real(&self, id: i32) -> Result<Option<auth_service::Permission>, DbErr> {
        Entity::delete_by_id(id).exec(self.db).await?;
        Ok(None)
    }

    // 保存并更新
    async fn save(&self, mut data: auth_service::Permission) -> Result<Option<auth_service::Permission>, DbErr> {
    ...
    // 批量删除
    async fn batch_delete(&self, dtos: Vec<auth_service::Permission>) -> Result<Option<i32>, DbErr> {
        let mut ids = vec![];
        for dto in dtos {
            ids.push(dto.id);
        }
