...
use model::folder::{ActiveModel, Column, Entity, Model};
...
impl<'a> Business<'a> {
    ...
    fn decode(&self, dto: public_service::Folder) -> Model {
    ...
    fn encode(&self, model: Model) -> public_service::Folder {
        public_service::Folder {
            id: model.id,
    ...
    pub async fn list(
        &self,
        filter: Option<public_service::FolderFilter>,
        mut sorts: Vec<public::Sort>,
        pager: Option<public::Pager>,
    ) -> public_service::ListFolderReply {
        ...
        let header = if pager.disabled {
            ...
            match query.all(self.db).await {
                ...
                Err(e) => common::header::err_reply(
                    ...
                    ECode::PublicServiceErrorListFolder,
                    format!("list folder failed: {}", e),
                ),
            }
        } else {
            // 分页
        ...
        public_service::ListFolderReply { header, pager: Some(pager), data: dtos }
    }

    pub async fn operate(&self, operate: Operation, data: Option<public_service::Folder>) -> public_service::OperateFolderReply {
        ...
        public_service::OperateFolderReply { header, data }
    }

    pub async fn batch_operate(&self, operate: Operation, data: Vec<public_service::Folder>) -> public_service::BatchOperateFolderReply {
        ...
        public_service::BatchOperateFolderReply { header }
    }


    // 插入
    async fn insert(&self, dto: public_service::Folder) -> Result<Option<public_service::Folder>, DbErr> {
    ...
    // 更新
    async fn update(&self, dto: public_service::Folder) -> Result<Option<public_service::Folder>, DbErr> {
    ...
    // 逻辑删除
    async fn delete(&self, id: i32) -> Result<Option<public_service::Folder>, DbErr> {
    ...
    // 物理删除
    async fn delete_real(&self, id: i32) -> Result<Option<public_service::Folder>, DbErr> {
        Entity::delete_by_id(id).exec(self.db).await?;
        Ok(None)
    }

    // 保存并更新
    async fn save(&self, mut data: public_service::Folder) -> Result<Option<public_service::Folder>, DbErr> {
    ...
    async fn batch_save(
        &self,
        dtos: Vec<public_service::Folder>,
    ...
    // 批量删除
    async fn batch_delete(&self, dtos: Vec<public_service::Folder>) -> Result<Option<i32>, DbErr> {
        let mut ids = vec![];
        for dto in dtos {
            ids.push(dto.id);
        }
